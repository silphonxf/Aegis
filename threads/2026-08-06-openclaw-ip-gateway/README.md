...
截至 2026-08-09，源码开发与本机专项验证已完成：

- Aegis HTTPS 工具网关运行于开发端口 18791；
- OpenClaw 插件 `aegis-tools` 源码版本为 `1.3.0`；
...
## 2026-08-09 IP 情报清单更新

- 18791 管理控制台支持一次输入一个或多个 IPv4，自动去重并形成情报清单；
- 清单展示恶意结论、位置、危险类型、严重度、可信度和情报摘要；
- 管理员可逐项勾选、只选建议封禁、全选或清空，再封禁明确选中的 IP；
- 飞书分析卡片支持逐项选择/取消，并显示“确认已选 N 个”；
- `aegis_ip_prepare_block` 支持 `selection=selected` 和 `selected_ips`；
- 后端拒绝任何不属于原研判批次清单的自定义 IP；
- 济南二次确认、批次所有权、dry-run 和永久封禁总开关保持不变。

## 提交前验证

- 后端全量测试：`91 passed`；
- OpenClaw/安全响应/威胁情报专项测试：`24 passed`；
