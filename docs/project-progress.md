...
### 4) 移动端 AI 自检改为显式触发并保存报告文件

已完成：
- 移动端系统自检页打开后只展示状态、配置和报告记录，不再自动开始 AI 自检
- 系统切换时不再自动调用自检接口，只刷新当前系统的最近报告列表
- 新增“开始 AI 自检”按钮，点击后才生成 AI 自检报告
- 新增定时 AI 自检配置区：
  - 定时间隔
  - 下次执行时间
  - 保存定时配置
- 定时配置保存在移动端浏览器本地；浏览器运行且登录有效时，到点自动对所选系统发起 AI 自检
- 后端 `/api/v1/selfchecks/run` 每次执行后保存报告文件
- 报告文件保存目录：`backend/selfcheck_reports/`
- 报告文件命名包含系统与时间：`<system_code-system_name>_<YYYYmmddHHMMSS>.json`
- 新增 `/api/v1/selfchecks/reports`：
  - 默认展示最近 6 小时报告
  - 支持按系统、开始时间、结束时间搜索
  - 按用户可见系统做权限过滤
- 报告文件清理：运行/查询时删除超过 7 天的报告，节省空间并减少搜索范围

验证结果：
- `node --check frontend-mobile/app.js` → 通过
- `python3 -m py_compile backend/app/api/selfchecks.py` → 通过

当前剩余：
- pytest 被当前沙箱 `bwrap: loopback: Failed RTM_NEWADDR` 拦截，未能执行后端回归测试
- 定时 AI 自检当前是移动端浏览器运行时触发；后续如需无人值守，应迁移为后端定时任务并由管理端统一配置
...
已完成：
- 资产与日常运维侧边栏新增“巡检点”入口
- 工作台“机房巡检配置与记录”跳转从旧承接页调整到巡检点配置页
- 新增管理端巡检点列表页，支持：
  - 关键词筛选
  - 启用状态筛选
  - 新增巡检点
...
- 新增巡检点弹窗，支持配置：
  ...
  - 位置说明
  - 启用状态
- 新增 `frontend-admin/js/inspection-points.js`，复用已有后端巡检点 CRUD 接口
- 点位表单的机房、系统下拉来自管理端共享主数据接口，延续“管理端配置、移动端消费”的原则

验证结果：
- `node --check frontend-admin/js/inspection-points.js` → 通过
